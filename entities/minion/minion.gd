...
func _acquire_nearest_enemy() -> void:
	...
	for node: Node in get_tree().get_nodes_in_group(&"live_enemies"):
		var candidate: Enemy = node as Enemy
...
func _follow_owner() -> void:
	var offset: Vector2 = owner_hero.global_position - global_position
